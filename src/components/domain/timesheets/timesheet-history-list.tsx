...
export function TimesheetHistoryList({
  ...
  const [details, setDetails] = useState<Map<string, ExpandedDetail>>(
    new Map(),
  );
  ...
  return (
    <div className="space-y-3">
      {timesheets.map((ts) => {
        ...
        const statusConfig =
          TIMESHEET_STATUS_CONFIG[ts.status as TimesheetStatus];
                    ...
                    <table className="min-w-full divide-y divide-gray-100">
                      <thead>
                        <tr>
                          ...
                          <th className="pl-4 py-2 text-left text-xs font-medium uppercase text-muted-foreground">
                            Notes
                          </th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-50">
                        {detail.data.time_entries
                          ...
                          .map((entry) => {
                            const typeConfig =
                              TIME_ENTRY_TYPE_CONFIG[
                                entry.entry_type as TimeEntryType
                              ];
                            return (
                              <tr key={entry.id}>
                                <td className="whitespace-nowrap py-2 pr-4 text-sm text-foreground">
                                ...
                                <td className="pl-4 py-2 text-sm text-muted-foreground">
                                  {entry.notes || "–"}
                                </td>
                              </tr>
                            );
                ...
                {detail?.data && (
                  <div className="mt-4 flex flex-wrap gap-[var(--density-card-padding)] border-t border-gray-100 pt-3 text-xs text-muted-foreground">
                    {detail.data.submitted_at && (
                      <span>
                        Submitted:{" "}
                        {new Date(detail.data.submitted_at).toLocaleDateString(
                          "en-AU",
                          {
                            day: "numeric",
                            month: "short",
                    ...
                    {detail.data.approved_at && (
                      <span>
                        Approved:{" "}
                        {new Date(detail.data.approved_at).toLocaleDateString(
                          "en-AU",
                          {
                            day: "numeric",
                            month: "short",
                    ...
                    {detail.data.synced_at && (
                      <span>
                        Synced:{" "}
                        {new Date(detail.data.synced_at).toLocaleDateString(
                          "en-AU",
                          {
                            day: "numeric",
                            month: "short",
