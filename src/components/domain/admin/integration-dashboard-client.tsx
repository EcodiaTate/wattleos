        ...
        return (
          <div
            ...
            className={`rounded-lg border bg-background shadow-sm transition-all ${
              config?.is_enabled ? "border-success/30" : "border-border"
            }`}
          >
            {/* Card header */}
              ...
              <div className="flex items-center gap-2">
                {config?.is_enabled && (
                  <button
                    ...
                    className="rounded-md border border-border px-3 py-1.5 text-xs font-medium text-foreground hover:bg-background transition-colors"
                  >
                    Sync Logs
                  </button>
                )}
                <GlowTarget id={`admin-btn-connect-${provider.key}`} category="button" label={`Connect ${provider.label}`}>
                  <button
                    onClick={() =>
                      setExpandedProvider(isExpanded ? null : provider.key)
                    }
                    disabled={!provider.implemented}
                    className="rounded-md bg-primary px-3 py-1.5 text-xs font-medium text-primary-foreground hover:bg-primary disabled:cursor-not-allowed disabled:opacity-50 transition-colors"
                  >
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3 border-t border-border pt-4">
        <button
          onClick={handleSave}
          disabled={isSaving}
          className="rounded-lg bg-primary px-4 py-2 text-sm font-medium text-primary-foreground shadow-sm hover:bg-primary disabled:opacity-50 transition-colors"
        >
        ...
        <button
          onClick={handleTest}
          disabled={isTesting}
          className="rounded-lg border border-border px-4 py-2 text-sm font-medium text-foreground hover:bg-background disabled:opacity-50 transition-colors"
        >
          {isTesting ? "Testing..." : "Test Connection"}
        </button>
        {existingConfig && (
          <GlowTarget id={`admin-btn-disconnect-${provider.key}`} category="button" label={`Disconnect ${provider.label}`}>
            <button
              onClick={handleDelete}
              className="ml-auto rounded-lg border border-destructive/30 px-4 py-2 text-sm font-medium text-destructive hover:bg-destructive/10 transition-colors"
            >
              Remove Integration
            </button>
          </GlowTarget>
...
function CredentialInput({
  ...
  const baseClasses =
    "mt-1 block w-full rounded-lg border border-border px-3 py-2 text-sm focus:border-ring focus:outline-none focus:ring-1 focus:ring-ring";
