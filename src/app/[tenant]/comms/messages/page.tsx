// src/app/[tenant]/comms/messages/page.tsx
//
      ...
      {channels.length === 0 ? (
        <div className="rounded-lg border-2 border-dashed border-gray-200 p-12 text-center">
          ...
          <p className="mt-1 text-sm text-gray-500">
            Channels will appear here when class groups are set up or you
...
function ChannelRow({
  ...
  return (
    <Link
      href={`/${tenantSlug}/comms/messages/${channel.id}`}
    ...
    >
      ...
      <div
        className={`flex h-10 w-10 items-center justify-center rounded-full text-sm font-medium ${
          CHANNEL_TYPE_COLORS[channel.channel_type] ?? "bg-gray-100 text-gray-700"
        }`}
      >
...
function NewMessageButton({ tenantSlug }: { tenantSlug: string }) {
  return (
    <Link
      href={`/${tenantSlug}/comms/messages/new`}
