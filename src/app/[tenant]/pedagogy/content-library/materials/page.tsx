// src/app/(app)/[tenant]/pedagogy/content-library/materials/page.tsx
//
// ============================================================
        ...
        <div className="rounded-lg border border-dashed border-border p-12 text-center">
          ...
          <div className="mt-6 flex flex-wrap justify-center gap-2">
            {POPULAR_MATERIALS.map((material) => (
              <Link
                key={material}
                href={`/${tenant}/pedagogy/content-library/materials?q=${encodeURIComponent(material)}`}
...
function groupResults(results: MaterialSearchResult[]): ResultGroup[] {
  ...
  for (const r of results) {
    // Prefer grouping by instance (school's fork), fall back to template
    ...
    <div className="rounded-lg border border-border overflow-hidden">
      ...
      <div className="divide-y divide-border">
        {group.results.map((result) => (
          <Link
            key={result.node_id}
            href={`/${tenant}/pedagogy/content-library/node/${result.node_id}`}
...
/** Colored dot for curriculum node level */
