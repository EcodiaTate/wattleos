...
export async function getMyTimesheets(): Promise<
  ActionResponse<Array<Timesheet & { pay_period_name: string }>>
> {
  try {
    const context = await requirePermission(Permissions.LOG_TIME);
    const supabase = await createSupabaseServerClient();
...
export async function listTimesheetsForPeriod(
  ...
  ActionResponse<
    Array<Timesheet & { user_name: string; avatar_url: string | null }>
  >
> {
  try {
    await requirePermission(Permissions.APPROVE_TIMESHEETS);
...
export async function getTimesheetDetail(
  ...
  try {
    ...
    const isApprover = context.permissions.includes(
    ...
    const result: TimesheetWithEntries = {
      ...
      time_entries: ((entries ?? []) as Array<Record<string, unknown>>).map(
        (e) => ({
          id: e.id as string,
...
export async function batchApproveTimesheets(
  ...
  ActionResponse<{ approved: number; failed: number; errors: string[] }>
> {
  try {
    const context = await requirePermission(Permissions.APPROVE_TIMESHEETS);
    const supabase = await createSupabaseServerClient();
