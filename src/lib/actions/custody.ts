...
export async function createCustodyRestriction(
  ...
  try {
    ...
    if (!input.restriction_type)
      return failure("Restriction type is required", "VALIDATION_ERROR");
    if (!input.effective_date)
      return failure("Effective date is required", "VALIDATION_ERROR");
...
export async function updateCustodyRestriction(
  ...
  try {
    ...
    if (input.restricted_person_name !== undefined)
      updateData.restricted_person_name = input.restricted_person_name.trim();
    if (input.restriction_type !== undefined)
      updateData.restriction_type = input.restriction_type;
    if (input.court_order_reference !== undefined)
      updateData.court_order_reference =
        input.court_order_reference?.trim() || null;
    if (input.court_order_doc_url !== undefined)
      updateData.court_order_doc_url = input.court_order_doc_url || null;
