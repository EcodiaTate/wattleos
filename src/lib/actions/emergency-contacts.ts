...
export async function listEmergencyContacts(
  ...
  try {
    ...
    const message =
      err instanceof Error ? err.message : "Failed to list emergency contacts";
...
export async function createEmergencyContact(
  ...
  try {
    ...
    if (!input.phone_primary?.trim())
      return failure("Primary phone is required", "VALIDATION_ERROR");
    ...
    const message =
      err instanceof Error ? err.message : "Failed to create emergency contact";
    return failure(message, "UNEXPECTED_ERROR");
  }
}
...
export async function updateEmergencyContact(
  ...
  try {
    ...
    if (input.phone_primary !== undefined)
      updateData.phone_primary = input.phone_primary.trim();
    if (input.phone_secondary !== undefined)
      updateData.phone_secondary = input.phone_secondary?.trim() || null;
    if (input.email !== undefined)
      updateData.email = input.email?.trim() || null;
    if (input.priority_order !== undefined)
      updateData.priority_order = input.priority_order;
    ...
    const message =
      err instanceof Error ? err.message : "Failed to update emergency contact";
    return failure(message, "UNEXPECTED_ERROR");
  }
}
...
export async function deleteEmergencyContact(
  ...
  try {
    ...
    const message =
      err instanceof Error ? err.message : "Failed to delete emergency contact";
    return failure(message, "UNEXPECTED_ERROR");
  }
}
