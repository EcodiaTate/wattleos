...
export async function getChildObservations(
  ...
  ActionResponse<ChildObservation[]> & {
...
> {
  try {
    ...
    for (const link of outcomeLinks ?? []) {
      const node = link.node as unknown as {
        id: string;
        title: string;
        level: string;
      } | null;
...
export async function getChildMastery(
  ...
  try {
    ...
    for (const record of masteryRecords ?? []) {
      const node = record.node as unknown as {
        id: string;
        instance_id: string;
      };
    ...
    const summaries: ChildMasterySummary[] = Array.from(
      instanceMap.entries(),
